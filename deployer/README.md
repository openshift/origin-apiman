...
API objects to implement APIMan. There are a number of
...
cluster is not using the multitenant plugin, then any project can be used.
...
* `ca.crt` - A certificate for the internal component CA.
...
The APIMan deployment also requires the `apiman-gateway` service account
